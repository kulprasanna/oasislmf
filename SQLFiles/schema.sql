...
INSERT #TempVariables ([VariableType], [VariableName]) VALUES ('FileLocationShiny','%FILE_LOCATION_SHINY%')
...
ALTER TABLE [dbo].[ProfileValueDetail] WITH CHECK ADD  CONSTRAINT [FK_ProfileValueDetail_PerilID] FOREIGN KEY([PerilID]) REFERENCES [dbo].[Peril] ([PerilID])
ALTER TABLE [dbo].[ProfileValueDetail] WITH CHECK ADD  CONSTRAINT [FK_ProfileValueDetail_CoverageTypeID] FOREIGN KEY([CoverageTypeID]) REFERENCES [dbo].[CoverageType] ([CoverageTypeID])
...
SELECT	@UserId, @BFEUserName, @CompanyID, @BFEUserLogin, @BFEUserPassword, @BFEUserDept, 0
SELECT	@UserId
GO
...
Create Table #Item (ItemId int identity(1,1) primary key, InterestGroupId int, InterestRiskID int, InterestSubRiskID int, InterestExposureID int,
					...
					OccCode nvarchar(255) NULL, AccountNumber nvarchar(255) NULL, SubLimitRef nvarchar(255) NULL, ElementDimensionID int NULL, ProgID int NULL)
...
Insert Into	#Item (InterestGroupId, InterestRiskID, InterestSubRiskID, InterestExposureID,GroupId, LocId, PerilId, CoverageTypeId, TIV, 
					...
					OccScheme, OccCode, AccountNumber,SubLimitRef,ElementDimensionID,ProgID)
...
if @modeltypeid = 1
	begin
		...
		delete from #item where AreaPerilId is null and VulnerabilityId is null
		delete from #item where TIV = 0
...
if @modeltypeid = 2 
	begin
		...
		--remove locs ith no wind match
		delete from #item where locid in (select distinct locid from #item where AltItemID%8 in (1,2,3,4) and AreaPerilId is null and VulnerabilityId is null)
		...
		--remove missing storm surge items
		--delete from #item where AltItemID%8 in (5,6,7,0) and AreaPerilId is null and VulnerabilityId is null
...
Select	Distinct AltItemID AS item_id,
		...
		dense_rank() over (order by OccScheme + '-' + OccCode) AS lob_id,
		OccScheme + '-' + OccCode AS lob_desc,
...
CREATE TABLE #FM (ITEM_ID int NULL, ALTITEM_ID int NULL, LEVEL_ID int NULL, LAYER_ID int NULL, AGG_ID int NULL, POLICYTC_ID int NULL, DEDUCTIBLE FLOAT null, LIMIT FLOAT null, SHARE_PROP_OF_LIM FLOAT null)
...
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,1,1,DENSE_RANK() OVER (ORDER BY coverageid) AS AGG_ID,NULL,NULL,NULL,NULL from #Item
	
--level 2, combined level
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,2,1,DENSE_RANK() OVER (ORDER BY case when coveragetypeid =4 then 2 else 1 end, locid) AS AGG_ID,NULL,NULL,NULL,NULL from #Item
	
--level 3, location level
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,3,1,DENSE_RANK() OVER (ORDER BY locid) AS AGG_ID,NULL,NULL,NULL,NULL from #Item
	
--level 4, sublimit level
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,4,1,DENSE_RANK() OVER (ORDER BY sublimitref) AS AGG_ID,NULL,NULL,NULL,NULL from #Item
	
--level 5, policy level
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,5,1,DENSE_RANK() OVER (ORDER BY PolicyID) AS AGG_ID,NULL,NULL,NULL,NULL from #Item
	
--level 6, layer level
insert into #FM (ITEM_ID,ALTITEM_ID,LEVEL_ID,LAYER_ID,AGG_ID,POLICYTC_ID,DEDUCTIBLE,LIMIT,SHARE_PROP_OF_LIM)
	select ItemId,AltItemID,6,PL.LayerNumber,DENSE_RANK() OVER (ORDER BY I.PolicyID) AS AGG_ID,NULL,NULL,NULL,NULL from #Item as i
...
select	distinct pc.PolicyCoverageID,
		...
		pvd.PerilID,
		pe.FieldID,
		case when pe.FieldID in (15,16,17,18,19,20) then pcv.FieldValue else cast(0 as float) end as FieldValue
...
update #fm set LIMIT = FieldValue from (
				select	i.ItemId,
						case when i.coveragetypeid = 3 then 0 else c.FieldValue end as FieldValue
				from	#exposures as e 
				join	#coverages as c 
							on	e.CoverageItemID = c.CoverageItemID 
				join	#item as i
...
create table #PolTC (POLICYTC_ID int identity(1,1), deductible float null, limit float null, SHARE_PROP_OF_LIM float null)
insert into #PolTC
select 0,0,0
union
select distinct deductible, limit, SHARE_PROP_OF_LIM from #FM



update	#FM set POLICYTC_ID = A.POLICYTC_ID 
from	#PolTC AS A
where	#FM.deductible = a.deductible
...
set		CALCRULE_ID = CASE	
			WHEN LIMIT = 0 AND SHARE_PROP_OF_LIM = 0 Then 12
			WHEN LIMIT > 0 AND SHARE_PROP_OF_LIM = 0 Then 1
			ELSE 2 END
...
Select	Distinct Item_Id,
		DENSE_RANK() OVER(ORDER BY AGG_ID,LAYER_ID) AS OUTPUT_ID,
		AGG_ID,
		LAYER_ID,
		PolicyID
From	(
...
from	#item
where	#FMXRef.item_id = #item.itemid
...
Select Distinct OUTPUT_ID,AGG_ID,LAYER_ID From #FMXRef order by AGG_ID

--FM Dict
insert into OasisFMDICT (output_id,agg_id,layer_id,policy_name,layer_name)
Select Distinct OUTPUT_ID,AGG_ID,LAYER_ID,policy_name,layer_name From #FMXRef
...
From	(
		Select	Level_ID,
				Sum(CASE When PolicyTC_ID = 1 Then 1 Else 0 End) AS StubPolTC,
...
into	#level
from	OasisFM_PROGRAMME

update OasisFM_PROGRAMME set LEVEL_ID = newLEVEL_ID from #level where OasisFM_PROGRAMME.LEVEL_ID = #level.LEVEL_ID
update OasisFM_POLICYTC  set LEVEL_ID = newLEVEL_ID from #level where OasisFM_POLICYTC.LEVEL_ID  = #level.LEVEL_ID

--select * from #Item order by CoverageId
--select * from #fm
...
Set		@SQL = 'Select output_id,agg_id,layer_id
From	OpenRowset(''MSDASQL'',''Driver={Microsoft Access Text Driver (*.txt, *.csv)};
          DefaultDir='+@fmxreflocationname+';Extended Properties="text;HDR=YES;FMT=Delimited'',
          ''select * from '+@fmxreffilename+''')'

Create Table TempFMDict (output_id int NULL, agg_id int NULL, layer_id int null)
...
begin
	...
	if @SummaryLevelName = 'Policy_Id'
		begin
			set @sql = 'select distinct output_id, item_id as summary_id, '+ convert(nvarchar,@summarysetid) +' as summaryset_id from TempItemDict 
						join TempFMDict on TempItemDict.item_id = TempFMDict.agg_id'
		end
	else
		begin
			set @sql = 'select distinct output_id, ' + @SummaryLevelName + ' as summary_id, '+ convert(nvarchar,@summarysetid) +' as summaryset_id from TempItemDict 
						join TempFMDict on TempItemDict.item_id = TempFMDict.agg_id'
		end
...
--populate
Set @SQL = '
Select	ROW_NUMBER () OVER (Order By LocId,PerilId,CoverageId) + convert(int,' + convert(nvarchar,@ProgOasisKeyId) + ') AS ProgOasisKeyId,
		' + convert(nvarchar,@ProgOasisId) + ' AS ProgOasisId,
		LocId,
		case when convert(nvarchar,PerilId) = ''W'' Then 1 
			when convert(nvarchar,PerilId) = ''S'' Then 2
			Else PerilId End AS PerilId,
		CoverageId,
		AreaPerilId,
		VulnerabilityId 
From   OpenRowset(''MSDASQL'',''Driver={Microsoft Access Text Driver (*.txt, *.csv)};
...
and /*LocationDesc = case when @ProcessRunID <> -1 then 'ProcessRun_' + convert(varchar,@ProcessRunId)
...
CREATE procedure [dbo].[updateUser] 
			@BFEUserID int, 
			@BFEUserName nvarchar(20), 
			@CompanyID int, 
			@BFEUserLogin nvarchar(20), 
			@BFEUserPassword nvarchar(20), 
			@BFEUserDept nvarchar(20)
AS
SET NOCOUNT ON;
----------------------------------------------------------------------------
--log database usage
...
SET BFEUserName = @BFEUserName,
	...
	BFEUserPassword = @BFEUserPassword,
...
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@DemandSurge),		ParameterRunFormat = 'bool'		Where ParameterRunName = 'demand_surge'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@LeakageFactor),	ParameterRunFormat = 'float'	Where ParameterRunName = 'leakage_factor'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@PerilWind),		ParameterRunFormat = 'bool'		Where ParameterRunName = 'peril_wind'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@PerilSurge),		ParameterRunFormat = 'bool'		Where ParameterRunName = 'peril_surge'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@PerilQuake),		ParameterRunFormat = 'bool'		Where ParameterRunName = 'peril_quake'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@PerilFlood),		ParameterRunFormat = 'bool'		Where ParameterRunName = 'peril_flood'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@SessionID),		ParameterRunFormat = 'int'		Where ParameterRunName = 'session_id'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@EventSetID),		ParameterRunFormat = 'str'		Where ParameterRunName = 'event_set'
Update #ModelParameters Set ParameterRunValue = convert(nvarchar,@EventOccurrenceID),ParameterRunFormat = 'str'		Where ParameterRunName = 'event_occurrence_id'
...
INSERT [dbo].[BFEUser] ([BFEUserID], [BFEUserName], [CompanyID], [BFEUserLogin], [BFEUserPassword], [BFEUserDept], [Deleted]) VALUES (1, N'Admin', 1, N'Admin', N'password', N'System', 0)
